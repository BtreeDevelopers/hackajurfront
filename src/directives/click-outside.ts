...
type HTMLElementPerson = {
  _clickOutside: any;
} & HTMLElement;

function directive(
  e: MouseEvent,
  el: HTMLElementPerson,
  binding: ClickOutsideDirectiveBinding
) {
  const handler =
...
function handleShadow(el: HTMLElementPerson, callback: Function): void {
  const root = attachedRoot(el);
...
export const ClickOutside = {
  ...
  // clicks on body
  mounted(el: HTMLElementPerson, binding: ClickOutsideDirectiveBinding) {
  ...
  unmounted(el: HTMLElementPerson, binding: ClickOutsideDirectiveBinding) {
    if (!el._clickOutside) return;

    handleShadow(el, (app: HTMLElementPerson) => {
